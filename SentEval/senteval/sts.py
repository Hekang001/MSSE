...
class STSEval(object):
    def loadFile(self, fpath):
        ...
        for dataset in self.datasets:
            ...
            # max_length = max(max(len(s.split()) for s in sent1), max(len(s.split()) for s in sent2))
            # sent1 = np.array([s.split() + ['<PAD>'] * (max_length - len(s.split())) for s in sent1])[not_empty_idx]
            # sent2 = np.array([s.split() + ['<PAD>'] * (max_length - len(s.split())) for s in sent2])[not_empty_idx]
    ...
    def run(self, params, batcher):
        ...
        for dataset in self.datasets:
            ...
            # results[dataset]['alignment'] = align
            # results[dataset]['uniformity'] = unif
